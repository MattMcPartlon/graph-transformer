""""Fully differentiable representation of rigid transformations"""
...
class Rigids:
    ...
    def rotate(self, points):
        ...
        quats = self.quaternions if points.ndim == 3 \
            else self.quaternions.unsqueeze(-2)
    ...
    def apply_by_rot(self, points):
        """Apply rigid by using underlying rotation rather than quats"""
        quats, trans = self.quaternions, self.translations
        if points.ndim == 4:
            quats, trans = quats.unsqueeze(-2), trans.unsqueeze(-2)
    ...
    def clone(self):
        """Clone the underlying rigid object"""
        return Rigids(
