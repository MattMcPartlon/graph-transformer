...
class TriangleMul(nn.Module):
    ...
    def forward(self, edges, mask) -> Tensor:
        ...
        out_gate = torch.sigmoid(self.gate_out_proj(normed_edges))
        if exists(mask):
...
class TriangleAttn(nn.Module):
    ...
    def __init__(
        ...
        self.residual = self.residual = default(residual, lambda x, res: x)
        self.attn_fn = self._attn_starting if starting else self._attn_ending
    ...
    def forward(self, edges: Tensor, mask: Optional[Tensor]) -> Tensor:
        ...
        g = torch.sigmoid(self.to_g(normed_edges))
...
class PairUpdateBlock(nn.Module):
    ...
    def __init__(self,
        ...
        TriMul = lambda incoming: List([
            ...
            mul_residual if do_tri_mul else None,
        ...
        TriAttn = lambda starting: List([
            ...
            attn_residual if do_tri_attn else None,
    ...
    def forward(self, node_feats: Tensor, pair_feats: Tensor, mask: Optional[Tensor]) -> Tensor:
        ...
        for net, residual, dropout in self.layers[1:]:
            if not exists(net):
                continue
            out = checkpoint.checkpoint(net.forward, pair_feats, mask) \
                if self.do_checkpoint else net(pair_feats, mask)
            pair_feats = residual(dropout(out), pair_feats)
        return self.transition(self.dropout(pair_feats))
